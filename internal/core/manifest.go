...
func getManifest(version string) (*types.VersionManifest, error) {
	...
	manifestURLs := []string{
		fmt.Sprintf("https://github.com/hyperledger-firefly/firefly/releases/download/%s/manifest.json", version),
		fmt.Sprintf("https://raw.githubusercontent.com/hyperledger-firefly/firefly/%s/manifest.json", version),
	}
	var lastErr error
